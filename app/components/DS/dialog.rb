class DS::Dialog < DesignSystemComponent
  ...
  def merged_opts
    ...
    data[:controller] = [ "DS--dialog", "hotkey", "focus-trap", data[:controller] ].compact.join(" ")
    data[:DS__dialog_auto_open_value] = auto_open
    data[:DS__dialog_reload_on_close_value] = reload_on_close
    data[:action] = [ "mousedown->DS--dialog#clickOutside", "keydown.escape->DS--dialog#close", data[:action] ].compact.join(" ")
