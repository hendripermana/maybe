...
export default class extends Controller {
  ...
  async measureInteractionTimes() {
    ...
    for (const element of elements) {
      ...
      const elementId = element.id || `${elementType}-${Math.random().toString(36).substring(2, 9)}`
      ...
      // End measuring
      const duration = performanceMonitor.endMeasure(`interaction-${elementId}`)
  ...
  calculateUnusedCssPercentage() {
    if (!this.testResults.cssAnalysis || 
    ...
    return (
      (this.testResults.cssAnalysis.unusedRules / 
       this.testResults.cssAnalysis.totalRules) * 100
    ).toFixed(2)
  }
