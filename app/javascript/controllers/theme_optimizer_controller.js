...
export default class extends Controller {
  ...
  optimizeThemeSwitching() {
    if (this.hasThemeSwitchTarget) {
      this.themeSwitchTarget.addEventListener("click", this.handleThemeSwitch.bind(this))
    }
    
    // Listen for theme changes from other sources
    document.addEventListener("theme-changed", this.measureThemeSwitch.bind(this))
  }
  ...
  measureThemeSwitch(event) {
    ...
    if (switchTime > 200) {
      console.warn(`Theme switch to ${theme} took ${switchTime}ms, which is slower than the 200ms target`)
    }
  }
}