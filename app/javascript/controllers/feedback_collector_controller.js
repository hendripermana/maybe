...
export default class extends Controller {
  static targets = ["form", "type", "message", "page", "theme", "submitButton", "successMessage", "errorMessage"]
  ...
  connect() {
    ...
    if (this.hasThemeTarget) {
      this.themeTarget.value = document.documentElement.dataset.theme || 'light'
    }
  }
  ...
  async submitFeedback(event) {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Show error message
        if (this.hasErrorMessageTarget) {
          this.errorMessageTarget.textContent = result.errors?.join(', ') || 'Failed to submit feedback'
      ...
      // Show error message
      if (this.hasErrorMessageTarget) {
        this.errorMessageTarget.textContent = 'An error occurred while submitting your feedback'
        this.errorMessageTarget.classList.remove('hidden')
      }
    } finally {
