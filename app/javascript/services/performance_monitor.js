...
export default class PerformanceMonitor {
      ...
      try {
        const longTaskObserver = new PerformanceObserver((list) => {
          list.getEntries().forEach((entry) => {
            ...
            this.metrics.resources.longTasks = this.metrics.resources.longTasks || []
        ...
        const layoutShiftObserver = new PerformanceObserver((list) => {
          list.getEntries().forEach((entry) => {
            if (!entry.hadRecentInput) {
              ...
              this.metrics.resources.layoutShifts = this.metrics.resources.layoutShifts || []
        ...
        const resourceObserver = new PerformanceObserver((list) => {
          list.getEntries().forEach((entry) => {
            ...
            if (entry.initiatorType === 'link' || entry.initiatorType === 'script') {
              this.metrics.resources.entries = this.metrics.resources.entries || []
  ...
  endMeasure(componentId) {
    ...
    this.metrics.renders[componentId].average = 
      this.metrics.renders[componentId].totalTime / this.metrics.renders[componentId].counts
  ...
  trackInteraction(interactionId, callback) {
    ...
    this.metrics.interactions[interactionId].average = 
      this.metrics.interactions[interactionId].totalTime / this.metrics.interactions[interactionId].counts
    ...
    if (duration > 50) {
      console.warn(`Slow interaction detected for ${interactionId}: ${duration.toFixed(2)}ms`)
    }
