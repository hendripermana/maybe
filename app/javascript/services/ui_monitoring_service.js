...
export default class UiMonitoringService {
  ...
  static setupComponentErrorMonitoring() {
    ...
    window.addEventListener("error", (event) => {
      ...
      if (event.target && event.target.closest("[data-component]")) {
        const componentName =
          event.target.closest("[data-component]").dataset.component;
          ...
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              const errorElements = node.querySelectorAll(
                "[data-error], .error-boundary, .component-error",
              );
              if (errorElements.length > 0) {
                errorElements.forEach((element) => {
                  this.reportError(
                    "component_render_failure",
                    new Error(
  ...
  static reportError(errorType, error, context = {}) {
    ...
    if (
      window.Sentry &&
      SentryIntegrationService.isDuplicateError(error, context)
    ) {
  ...
  static sendToUiMonitoring(eventType, data) {
    ...
    document.dispatchEvent(
      new CustomEvent("ui-monitoring:event", {
    ...
    if (eventType === "performance_metric") {
      document.dispatchEvent(
        new CustomEvent("ui-monitoring:performance", {
          detail: {
