...
export default class CssPurge {
          ...
          if (this.options.preserveMediaQueries) {
            ...
            Array.from(rule.cssRules).forEach((mediaRule, mediaIndex) => {
              if (mediaRule.type === CSSRule.STYLE_RULE) {
                this.processStyleRule(mediaRule, rule, mediaIndex, unusedRules, rule)
              }
            })
          }
        }
  ...
  hasPreservedPrefix(selector) {
    return this.options.preservePrefixes.some(prefix => 
  ...
  getStats() {
    return {
      ...
      unusedPercentage: this.stats.totalRules > 0 
        ? (this.stats.unusedRules / this.stats.totalRules * 100).toFixed(2) + '%'
        : '0%'
    }
  }
}
